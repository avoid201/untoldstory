...
class BattleControllerCore:
    ...
    def _transition_to_input_phase(self) -> None:
        """Transition to INPUT phase."""
        self.state.phase = BattlePhase.INPUT
        self.state.waiting_for_input = True
        logger.info("✓ Transitioned to INPUT phase")
    
    def _transition_to_execution_phase(self) -> None:
        """Transition to EXECUTION phase."""
        self.state.phase = BattlePhase.EXECUTION
        self.state.waiting_for_input = False
        logger.info("✓ Transitioned to EXECUTION phase")
    
    def _transition_to_aftermath_phase(self) -> None:
        """Transition to AFTERMATH phase."""
        self.state.phase = BattlePhase.AFTERMATH
        self.state.waiting_for_input = False
        logger.info("✓ Transitioned to AFTERMATH phase")
    
    def _transition_aftermath_to_input(self) -> None:
        """Transition from AFTERMATH back to INPUT phase."""
        self.state.phase = BattlePhase.INPUT
        self.state.waiting_for_input = True
        logger.info("✓ Transitioned from AFTERMATH to INPUT phase")
