...
class BattleValidator:
    ...
    def validate_move(move: Dict[str, Any], monster: MonsterInstance) -> Tuple[bool, List[str]]:
        """
        Validates a move for a monster using Talent-System.
        ...
        try:
            ...
            # Convert to dict if it's a Move object
            if hasattr(move, 'id'):
                move_dict = {
                    'id': move.id,
                    'name': move.name,
            ...
            for prop in required_props:
                if prop not in move_dict:
                    errors.append(f"Move missing required property: {prop}")
            
            # Check power value
            if 'power' in move_dict and move_dict['power'] <= 0:
                errors.append("Move power must be positive")
            
            # Check accuracy
            if 'accuracy' in move_dict and (move_dict['accuracy'] < 0 or move_dict['accuracy'] > 100):
                errors.append("Move accuracy must be between 0 and 100")
            
            # TALENT-SYSTEM VALIDATION: Check if monster can use this move
            if not BattleValidator._validate_move_availability(move_dict, monster):
                errors.append(f"Monster {monster.name} cannot use move {move_dict.get('name', 'unknown')}")
    ...
    def _validate_move_availability(move_dict: Dict[str, Any], monster: MonsterInstance) -> bool:
        """
        Validate if monster can use this move through talents.
        
        Args:
            move_dict: Move data
            monster: Monster trying to use the move
        ...
        try:
            ...
            move_name = move_dict.get('name', '').lower()
            
            for available_move in available_moves:
                if (hasattr(available_move, 'id') and available_move.id == move_id) or \
                   (hasattr(available_move, 'name') and available_move.name.lower() == move_name):
                    return True
            
            return False
            
        except Exception as e:
            logger.error(f"Error validating move availability: {e}")
            return False
