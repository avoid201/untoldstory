...
from engine.systems.battle.error_recovery import with_battle_fallback, safe_battle_operation
...
class ActionProcessorBase(ABC):
    ...
    @with_battle_fallback({'success': False, 'message': 'Action execution failed', 'fallback': True}, 'move_execution')
    def safe_execute(self, action: BattleAction) -> Dict[str, Any]:
        """Safely execute action with comprehensive error recovery."""
        try:
            # Validate action first
            is_valid, errors = self.validate_action_detailed(action)
            if not is_valid:
                return self.create_error_result(
                    action.action_type.name if hasattr(action, 'action_type') else 'UNKNOWN',
            ...
            if not isinstance(result, dict):
                return self.create_error_result(
                    action.action_type.name if hasattr(action, 'action_type') else 'UNKNOWN',
                    "Action execution returned invalid result"
                )
