...
class Scene(ABC):
    ...
    def exit(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Release any surfaces
            for attr_name in dir(self):
                attr = getattr(self, attr_name)
                if isinstance(attr, pygame.Surface):
                    attr = None
            
            return None
        except Exception as e:
            print(f"Warning: Error during scene cleanup: {str(e)}")
            return None
...
class TransitionScene(Scene):
    ...
    def update(self, dt: float) -> None:
        ...
        # Initialize target scene at midpoint
        if self.progress >= 0.5 and hasattr(self, 'scene_kwargs') and not hasattr(self.to_scene, '_initialized'):
            try:
                self.to_scene.enter(**self.scene_kwargs)
                self.to_scene._initialized = True
            except Exception as e:
                print(f"Failed to initialize target scene during transition: {str(e)}")
                self.game.quit()
                return
