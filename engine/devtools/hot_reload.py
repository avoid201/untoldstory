...
class HotReloader:
    ...
    def file_changed(self, filepath: Path) -> None:
        ...
        if ext in self.file_handlers:
            ...
            try:
                ...
                if not self._verify_file_integrity(filepath, ext):
                    raise ValueError("Datei möglicherweise beschädigt")
                ...
                if hasattr(self.game, 'error_handler'):
                    self.game.error_handler.log_error(
                        severity='info',
                        message=f"Asset neu geladen: {filepath.name}",
                        source="HotReloader"
                    )
                
            except Exception as e:
                error_msg = f"Failed to reload {filepath.name}: {e}"
                print(error_msg)
    ...
    def _verify_file_integrity(self, filepath: Path, ext: str) -> bool:
        """
        Überprüfe Dateiintegrität.
