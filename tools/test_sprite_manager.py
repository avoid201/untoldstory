...
def test_sprite_loading():
    ...
    try:
        ...
        for tile_id in cache_info['tile_ids']:
            ...
            sprite_file = tile_info.get("sprite_file") if isinstance(tile_info, dict) else "unbekannt"
        ...
        for tile_id in test_tiles:
            ...
            name = tile_info.get('name', 'unbekannt') if isinstance(tile_info, dict) else 'unbekannt'
            if sprite is not None:
                print(f"  ✅ Tile {tile_id} ({name}): Sprite gefunden")
            else:
                print(f"  ❌ Tile {tile_id} ({name}): Kein Sprite")
