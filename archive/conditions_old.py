...
class ConditionManager:
    ...
    def inflict_primary(self, condition_id: str, duration: Optional[int] = None) -> Tuple[bool, str]:
        """
        ...
        """
        can_inflict, reason = self.can_inflict_primary(condition_id)
        if not can_inflict:
            return False, reason
        ...
        return True, f"{self.monster.name} wurde mit {condition.name} belegt!"
    ...
    def can_act(self) -> bool:
        ...
        if self.primary_condition:
            condition = StatusConditions.ALL_CONDITIONS.get(self.primary_condition)
            if condition and condition.prevents_action:
                return False
        
        # Check volatile conditions
        for condition_id in self.volatile_conditions:
            condition = StatusConditions.ALL_CONDITIONS.get(condition_id)
            if condition and condition.prevents_action:
                return False
    ...
    def _calculate_confusion_damage(self) -> int:
        """Calculate self-inflicted confusion damage using unified calculator."""
        try:
