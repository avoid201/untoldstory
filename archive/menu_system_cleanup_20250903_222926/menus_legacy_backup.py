...
class PartyMenu(MenuBase):
    """LEGACY: Use EnhancedPartyMenu instead."""
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        for i in range(6):
            ...
            if monster:
                # Monster info
                # Handle different monster object types
                if isinstance(monster, dict):
                    name = monster.get('nickname') or monster.get('name', 'Unbekannt')
                elif hasattr(monster, 'nickname'):
                    name = monster.nickname or (monster.species.name if hasattr(monster, 'species') and hasattr(monster.species, 'name') else 'Unbekannt')
                else:
                    name = 'Unbekannt'
                # Handle level access
                if isinstance(monster, dict):
                    level = monster.get('level', 1)
                    current_hp = monster.get('current_hp', 10)
                    max_hp = monster.get('max_hp', 10)
                else:
                    level = getattr(monster, 'level', 1)
                    current_hp = getattr(monster, 'current_hp', 10)
