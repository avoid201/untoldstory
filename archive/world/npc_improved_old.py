# ARCHIVED: 2025-01-03 - Duplicate of npc.py with outdated imports
"""
Verbessertes NPC-System mit Pathfinding und Bewegungsmustern

FUNCTIONS MERGED INTO: engine/world/npc.py
- ImprovedNPC functionality integrated into main NPC class
- RivalKlaus moved to appropriate location
- Pathfinding features preserved in main NPC system
"""
...
class ImprovedNPC:
    ...
    def _move_flee(self, player_pos: Tuple[int, int]):
        ...
        if dist <= self.sight_range:
            ...
            if not tile_manager.is_collision(new_x, new_y):
                self._set_target(new_x, new_y)
                self.facing = Direction.from_vector(dx, dy)
    ...
    def interact(self) -> Optional[Dict]:
        ...
        # Spieler anschauen - Richtung zum Spieler berechnen
        if hasattr(self.game, 'player') and self.game.player:
            player_x, player_y = self.game.player.x, self.game.player.y
            dx = player_x - self.x
            dy = player_y - self.y
            
            # Bestimme Richtung basierend auf größter Distanz
            if abs(dx) > abs(dy):
