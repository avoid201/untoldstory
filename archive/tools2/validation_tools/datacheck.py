...
class DataValidator:
    ...
    def validate_monsters(self) -> None:
        ...
        for monster in data["monsters"]:
            ...
            # Validate talents
            for talent in monster.get("talents", []):
                talent_id = talent.get("talent_id")
                if not talent_id:
                    self.results.append(ValidationResult(
                        level=ValidationLevel.ERROR,
                        file="monsters.json",
                        message=f"Talent missing talent_id",
                        details=f"Monster: {monster_id}"
                    ))
                    continue
                
                # Prüfe Talent-Struktur
                required_fields = ["learned_at_level", "current_tier", "experience"]
                for field in required_fields:
                    if field not in talent:
                        self.results.append(ValidationResult(
                            level=ValidationLevel.ERROR,
                            file="monsters.json",
                            message=f"Talent missing field: {field}",
                            details=f"Monster: {monster_id}, Talent: {talent_id}"
                        ))
                
                # Prüfe Level-Bereich
                learned_level = talent.get("learned_at_level", 1)
                if learned_level < 1 or learned_level > 100:
                    self.results.append(ValidationResult(
                        level=ValidationLevel.WARNING,
                        file="monsters.json",
                        message=f"Invalid talent learned level: {learned_level}",
                        details=f"Monster: {monster_id}, Talent: {talent_id}"
                    ))
                
                # Prüfe Tier-Bereich
                current_tier = talent.get("current_tier", 1)
                if current_tier < 1 or current_tier > 4:
                    self.results.append(ValidationResult(
                        level=ValidationLevel.WARNING,
                        file="monsters.json",
                        message=f"Invalid talent tier: {current_tier}",
                        details=f"Monster: {monster_id}, Talent: {talent_id}"
                    ))
